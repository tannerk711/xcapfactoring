...
export function buildTermsTable(x: Extraction): { field: string; value: string; found: string }[] {
  return [
    ...
    termRow('Monthly minimum forced w/ penalty', x.monthly_minimum.forced_with_penalty),
    termRow('Advance timing (client controls)', x.advance_timing.description ?? x.advance_timing.client_controls_timing, x.advance_timing.found),
    termRow('Funding speed', x.funding_speed.stated_timeline, x.funding_speed.found),
    termRow('Monitoring fee (USD/mo)', x.monitoring_fee.amount_usd, x.monitoring_fee.found),
    termRow('Wire fee (USD)', x.wire_fee_usd),
    termRow('ACH fee (USD)', x.ach_fee_usd),
    termRow('New-debtor credit fee (USD)', x.new_debtor_credit_fee.amount_usd, x.new_debtor_credit_fee.found),
    termRow('Due diligence fee (USD)', x.due_diligence_fee.amount_usd, x.due_diligence_fee.found),
